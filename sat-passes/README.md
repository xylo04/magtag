...
responsive rather than waiting on the time sync and N2YO lookups. The
last-updated time is remembered in `alarm.sleep_memory` across deep sleep; once
the refresh finishes, the page is redrawn if anything on it changed. After a
power cycle nothing is remembered yet, so it briefly reads `Updated unknown`.
